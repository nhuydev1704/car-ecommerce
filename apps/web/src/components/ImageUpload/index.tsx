...
const ImageUpload = ({ value, onChange }: any) => {
  return (
    <ReactImageUploading multiple value={value} onChange={onChange}>
      {({
        imageList,
        onImageUpload,
        ...
        <div>
            ...
            <div className="my-[15px] grid grid-cols-4 gap-3">
              {imageList.map((image, index: any) => (
                ...
                >
                  ...
                  />
                  {index === 0 && (
                    <div className="shadow-small absolute inset-x-0 bottom-0 z-10 flex justify-center overflow-hidden rounded-b-2xl bg-[#000000b2] before:rounded-xl">
          ...
          {errors?.maxNumber && (
            <span className="text-main2 font-bold">Tối đa 20 ảnh</span>
          )}
        </div>
      )}
    </ReactImageUploading>
